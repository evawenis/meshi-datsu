...
def reserve(driver, start_time):
    try:
        click_visible(
            driver, By.XPATH, f'//div[@data-start_unixtime="{start_time}"][@data-calendar_id="S001.{basho}..{start_time}.{start_time + 600}"]')
...
def select_pulldown(driver, kind, ident, string):
    ...
    Select(driver.find_element(kind, ident)
           ).select_by_value(string)
...
def scheduler(driver):
    ...
    print(
        f"waiting {diff}, until {datetime.strptime(schedule, '%Y-%m-%d %H:%M:%S.%f')}", file=sys.stderr)
...
def main():
    ...
    print('navigator.webdriver:', driver.execute_script(
        'return navigator.webdriver'), file=sys.stderr)
